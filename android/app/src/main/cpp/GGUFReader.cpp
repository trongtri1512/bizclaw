/**
 * BizClaw GGUF Reader — reads metadata from GGUF model files
 *
 * Extracts context_length and chat_template without loading the full model.
 * Useful for UI to show model info before downloading or loading.
 *
 * Reference: SmolChat-Android GGUFReader.cpp
 */
...
Java_vn_bizclaw_app_engine_GGUFReader_getGGUFContextNativeHandle(
...
extern "C" JNIEXPORT jlong JNICALL
Java_vn_bizclaw_app_engine_GGUFReader_getContextSize(
...
Java_vn_bizclaw_app_engine_GGUFReader_getChatTemplate(
        JNIEnv* env, jobject thiz, jlong nativeHandle) {
    gguf_context* ggufContext = reinterpret_cast<gguf_context*>(nativeHandle);
